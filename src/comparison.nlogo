;; THIS CODE IS MODIFIED FROM http://ccl.northwestern.edu/netlogo/models/ElFarol (see info tab).
;; It was used to generate fair comparisons based on the canonical el farol model (rather than reimplementing and making own design choices).
;; All rights belong to the original authors (Wilensky and Rand)
;; The main changes are
;; - Change overcrowding-threshold to a rate (0..1)
;; - Work for variable number of agents N
;; - Run for T ticks
;; These are commented (prefixed with 'ADDED') where changes are made


globals [
  attendance        ;; the current attendance at the bar
  attendance-rate   ;; ADDED: the current attendance at the bar (as a rate)
...
turtles-own [
  ...
  prediction-rate ;; ADDED: current prediction of the bar attendance (rate)
  attendance-history ;; ADDED: track decisions for each agent
  best-strategy-history ;; ADDED: track best strategy for each agent
  correctness    ;; ADDED: track how often correct decision is made
]
...
to setup
  ...
  set history n-values (memory-size * 2) [random N]
  ...
  set attendance-rate attendance / N ;; ADDED: for tracking rate so works with different N
...
to go
  ...
  ask turtles [
    ;; ADDED: check if agent was correct last time step
    if attend? = True and attendance-rate <= overcrowding-threshold [set correctness correctness + 1]
    if attend? = False and attendance-rate > overcrowding-threshold [set correctness correctness + 1]

    set prediction predict-attendance best-strategy sublist history 0 memory-size
    set prediction-rate prediction / N
    set attend? (prediction-rate <= overcrowding-threshold)  ;; true or false
    set attendance-history lput attend? attendance-history ;; ADDED: track attendance history
  ]
  ;; depending on their decision, the agents go to the bar or stay at home
  ask turtles [
    ifelse attend?
      [ move-to one-of bar-patches
  ...
  set attendance-rate attendance / N

  if attendance-rate > overcrowding-threshold [
...
to update-strategies
  ...
  let best-score memory-size * N + 1
  ...
  set best-strategy-history lput best-strategy best-strategy-history ;; ADDED: track strategy history
end
...
;;      ... + x(t - MEMORY-SIZE) * a(t - MEMORY-SIZE) + c * N,
...
to-report predict-attendance [strategy subhistory]
  ...
  report N * first strategy + sum (map [ [weight week] -> weight * week ] butfirst strategy subhistory)
end
...
<experiments>
  <experiment name="experiment" repetitions="3" runMetricsEveryStep="true">
    ...
    <steppedValueSet variable="overcrowding-threshold" first="0" step="0.1" last="0.2"/>
  </experiment>
</experiments>
@#$#@#$#@
@#$#@#$#@
